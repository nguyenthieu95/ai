...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar  3 03:35:40 2018

@author: thieunv

*) GradientDescentOptimizer (Standard - 2)

    elu +  elu / relu/ tanh/ sigmoid ===> failed/ failed / 0.55 / failed
    relu + elu / relu / tanh/ sigmoid ===>  0.58/ failed/ failed /failed
    tanh + elu / relu / tanh/ sigmoid ===>  failed/ failed/  /failed /failed
    sigmoid + elu / relu / tanh/ sigmoid  ==> failed
...
*) AdagradOptimizer   
     ...
     elu + elu / relu/  tanh/  sigmoid ===> failed/ failed / 0.38 (Nhin hinh thi xau) / failed

*) AdadeltaOptimizer
    
     elu + elu / relu/ tanh/ sigmoid ===> 0.52 (nhin hinh cung duoc)/ failed / 0.47 / failed
...
class Model(object):
    ...
    def build_bpnn_and_train(self):
        ...
        y_ = tf.nn.elu(tf.add(tf.matmul(hidden_out, W2), b2))
