...
- Activation: Relu, Relu/ relu, tanh  => MSE : 0.39
    + Loss giam tu tu, do do can nhieu epoch - 2800
    
- Activation: sigmoid, sigmoid/  Relu, Sigmoid / Sigmoid, Relu => MSE : 0.67 / 0.57
    + Loss giam rat tu tu, da~n den cac gia tri ~ nhau -> Error
    
- Activation: Elu, Elu ==> MSE: 0.38 (tot hon Relu,Relu)
- Activation: Relu, Elu ==> 0.42
...
class Model(object):
    ...
    def build_model_and_train(self):
        ...
        y_ = tf.nn.elu(tf.add(tf.matmul(hidden_out3, W4), b4))
...
learning_rates = [0.20]     #[0.005, 0.01, 0.025, 0.05, 0.10, 0.12, 0.15]   # [0.2]    
