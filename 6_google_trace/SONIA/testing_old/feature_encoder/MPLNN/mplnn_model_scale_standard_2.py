...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 27 11:43:46 2018

@author: thieunv

- MPLNN - Multiple Layer Neuron Network using Tensorflow
...
- Activation: Elu, Elu/ Relu, Elu/ Tanh, Elu/ Sigmoid, Elu ==> MSE: 0.44/ 0.45/ 0.38/ 0.39
...
- Activation: elu, tanh/ relu, tanh/ tanh, tanh/ sigmoid, tanh   => MSE: 0.40 (failed)/ 0.37/ 0.40/0.35 (tot nhat tu truoc den gio)
...
class Model(object):
    ...
    def preprocessing_data(self):
        ...
        training_set_transform = self.standard_scaler.fit_transform(training_set)
        testing_set_transform = self.standard_scaler.transform(testing_set)
    ...
    def build_model_and_train(self):
        ...
        W3 = tf.Variable(tf.random_normal([h2_size, h3_size], stddev=0.03, dtype=tf.float64), name="W3")
        b3 = tf.Variable(tf.random_normal([h3_size], dtype=tf.float64), name="b3")
        
        # and the weights connecting the hidden layer to the output layer
        W4 = tf.Variable(tf.random_normal([h3_size, y_size], stddev=0.03, dtype=tf.float64), name='W4')
        b4 = tf.Variable(tf.random_normal([y_size], dtype=tf.float64), name='b4')
    
        # calculate the output of the hidden layer
        hidden_out1 = tf.nn.elu( tf.add(tf.matmul(X, W1), b1) )
        hidden_out2 = tf.nn.elu( tf.add(tf.matmul(hidden_out1, W2), b2) )
        hidden_out3 = tf.nn.elu( tf.add(tf.matmul(hidden_out2, W3), b3) )
        # Forward propagation # now calculate the hidden layer output
        y_ = tf.nn.tanh(tf.add(tf.matmul(hidden_out3, W4), b4))
        ...
        for epoch in range(self.epoch):
            ...
            we1, bi1, we2, bi2, we3, bi3, we4, bi4 = sess.run([W1, b1, W2, b2, W3, b3, W4, b4])
            loss_plot.append(loss_epoch)
            print("Epoch: {}".format(epoch + 1), "loss = {}".format(loss_epoch))
            
        sess.close()
        self.w1, self.b1, self.w2, self.b2, self.w3, self.b3, self.w4, self.b4, self.loss_train = we1, bi1, we2, bi2, we3, bi3, we4, bi4, loss_plot
    ...
    def predict(self):
        ...
        W4 = tf.Variable(self.w4)
        b4 = tf.Variable(self.b4)
...
epochs = [3200] #[100, 250, 500, 1000, 1500, 2000]   # [500]                       
    ...
    for epoch in epochs:
        for batch_size in batch_sizes:
            for learning_rate in learning_rates:
                ...
                ann = Model(dataset_original, list_idx, epoch, batch_size, sliding, learning_rate)
                ann.fit()
