...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 15 12:03:43 2017
...
stimulation_level = [5.0]
distance_level = 0.1    # [0.1, 0.15, 0.2]
threshold_number = 2     #[2, 3, 4]
learning_rate = 0.005    #[0.02, 0.05, 0.1, 0.15, 0.2]
positive_number = 0.5      # [0.02, 0.05, 0.1, 0.15, 0.2]
...
#            delta_bias = -1 * positive_number * y_output * (1 - y_output) * (y_output - train_y[k]) * 0.00001
...
#            bias += delta_bias
    ...
    return (sorted_matrix_Wih, matrix_Who , bias, np.array(predict), list_loss)
...
#        testY =  GoogleTrace_orin_unnormal[train_size+sliding:length]
        ...
        trainX, trainY = data[0:1500], data[sliding:1500+sliding, 0:1]
        testX = data[1500:2000-sliding]
        testY = GoogleTrace_orin_unnormal[1500+sliding:2000]
...
#        trainX, trainY = data[0:1000], data[sliding:1000+sliding, 0:1]
#        testX = data[1000:1200-sliding]
#        testY = GoogleTrace_orin_unnormal[1000+sliding:1200]
        
#        trainX, trainY = data[0:1000], data[sliding:1000+sliding, 0:1]
#        testX = data[1000:1500-sliding]
#        testY = GoogleTrace_orin_unnormal[1000+sliding:1500]
            
        matrix_Wih, vector_Who, bias, predict, list_loss = mySONIA(trainX, trainY, testX, validation=valid, sliding=sliding,
