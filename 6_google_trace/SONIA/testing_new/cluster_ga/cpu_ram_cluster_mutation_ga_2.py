...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  2 14:04:37 2018
...
#        sum = reduce(add, individual)  # sum = reduce( (lambda tong, so: tong + so), individual )
    ...
    def evolve(self, pop, retain=0.2, random_select=0.05, mutate=0.01, alpha=1.5):
        
        # Take 80% of pop to Crossover
        number_of_crossover = int(self.pop_size * 0.8)
        list_index_crossover = sample( range(1, self.pop_size), number_of_crossover )
        list_crossover = [ copy.deepcopy(pop[i]) for i in list_index_crossover ]
        
        children1 = []
        while len(children1) < number_of_crossover:
            male = randint(0, number_of_crossover - 1)   # father
            female = randint(0, number_of_crossover - 1) # mother
            if male != female:
                m = list_crossover[male]
                f = list_crossover[female]
                son = []
                for i in range(len(m)):
                    minx = m[i] - alpha * abs(f[i] - m[i])
                    maxx = f[i] + alpha * abs(f[i] - m[i])
                    temp = (maxx - minx) * np.random.random_sample() + minx
                    son.append( temp )
                children1.append(np.array(son).reshape(-1, 1))
          
        # Take 10% of pop to Mutation
        number_of_mutation = int(self.pop_size * 0.1)
        list_index_mutation = sample( range(1, self.pop_size), number_of_mutation )
        list_mutation = [ copy.deepcopy(pop[i]) for i in list_index_mutation ]
        children2 = []     
        for i in range(number_of_mutation):
            father = copy.deepcopy(list_mutation[i])
            father[randint(0, len(father) - 1)] = np.random.random_sample()
            children2.append(father)
        
        pop.extend(children1)
        pop.extend(children2)
        
        sorted_list_pop = [ (self.fitness(indiv), indiv) for indiv in pop ]
        sorted_list_pop = sorted(sorted_list_pop, key=lambda temp : temp[0])
        sorted_pop = [ x[1] for x in sorted_list_pop ]
        parents = copy.deepcopy(sorted_pop[:self.pop_size])
        return parents
    
    
    def build_model_and_train(self):
        pop = self.population(self.pop_size, len(self.list_hu), self.value_min, self.value_max)
        fitness_history = [self.grade_pop(pop)]
        for i in range(self.gen_count):
            pop = self.evolve(pop, self.retain, self.random_select, self.mutate)
            temp = self.grade_pop(pop)
...
gen_count = 150     # So vong doi` 
value_min = -1      # value min of weight 
value_max = +1
retain=0.15          # % Giu lai trong qua trinh loc bo quan the
random_select=0.05  # % Lay tu phan bi loai bo, muc dich de lam tang tinh da dang cua quan the
