...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 14 13:18:07 2018
...
- 2input -> 2output 
- cluster ko dung he so

cpu_ram_cluster_mutation_pso_2out.py
...
#        list_transform.append(self.min_max_scaler2.fit_transform(list_split[0]))
#        list_transform.append(self.min_max_scaler1.fit_transform(list_split[1]))
            
        list_transform = self.min_max_scaler.fit_transform(np.reshape(dataset_original, (-1, 2)))
        print(self.min_max_scaler.data_max_)
...
#        print("done")
        ...
        dataset_y = list_transform[sliding:]   # Now we need to find dataset_X
        ...
        self.X_train, y_train = dataset_X[0:train_idx], dataset_y[0:train_idx]
        self.X_validation, y_validation = dataset_X[train_idx:validation_idx], dataset_y[train_idx:validation_idx]
        self.X_test, y_test = dataset_X[validation_idx:], dataset_y[validation_idx:]
...
#        print("Transform features done!!!")
    ...
    def get_average_mae(self, weightHO, X_data, y_data):
        mae_list = []        
        weightHO = np.reshape(weightHO, (-1, len(y_data)))
        dimension = y_data.shape[1]
        
        if self.activation == 0:    
            for i in range(0, len(X_data)):
                y_out = np.dot(X_data[i], weightHO)
                mae_list.append( np.sum(np.absolute(Model.elu_activation(y_out) - y_data[i])) / dimension)
                
        if self.activation == 1:
            for i in range(0, len(X_data)):
                y_out = np.dot(X_data[i], weightHO)
                mae_list.append( np.sum(np.absolute(Model.relu_activation(y_out) - y_data[i])) / dimension)
                
        if self.activation == 2:
            for i in range(0, len(X_data)):
                y_out = np.dot(X_data[i], weightHO)
                mae_list.append( np.sum(np.absolute(Model.tanh_activation(y_out) - y_data[i])) / dimension)
                
        if self.activation == 3:
            for i in range(0, len(X_data)):
                y_out = np.dot(X_data[i], weightHO)
                mae_list.append( np.sum(np.absolute(Model.sigmoid_activation(y_out) - y_data[i])) / dimension)
...
#        sum = reduce(add, individual)  # sum = reduce( (lambda tong, so: tong + so), individual )
    ...
    def build_model_and_train(self):
        ...
        """
        pop = self.population(self.pop_size, len(self.list_hu) * len(self.y_train), self.value_min, self.value_max)
        fitness_history = []
...
#pathsave = "/home/hunter/nguyenthieu95/ai/6_google_trace/SONIA/testing_new/cluster_pso/mutation/2m/result/cpu_ram/"
#fullpath = "/home/hunter/nguyenthieu95/ai/data/GoogleTrace/"
#filename = "data_resource_usage_twoMinutes_6176858948.csv"

pathsave = "/home/thieunv/university/LabThayMinh/code/6_google_trace/SONIA/testing_new/cluster_pso/mutation/2m/result/cpu_ram/"
...
stimulation_levels = [0.15]  #[0.10, 0.2, 0.25, 0.50, 1.0, 1.5, 2.0]  # [0.20]    
positive_numbers = [0.1] #[0.005, 0.01, 0.025, 0.05, 0.1, 0.15, 0.20]     # [0.1]
distance_levels = [0.25]
sliding_windows = [3] #[ 2, 3, 5]           # [3]  
list_num = [(12000, 20000)]
method_statistics = [0]
activations = [0]  # elu, relu, tanh, sigmoid

w_min = 0.4       # [0-1] -> [0.4-0.9]      Trong luong cua con chim
w_max = 0.9
c_couple = [(1.2, 1.2)]         #c1, c2 = 2, 2       # [0-2]   Muc do anh huong cua local va global 
# r1, r2 : random theo tung vong lap
# delta(t) = 1 (do do: x(sau) = x(truoc) + van_toc
pop_sizes = [100]      # Kich thuoc quan the
move_counts = [10]     # So lan di chuyen` 
