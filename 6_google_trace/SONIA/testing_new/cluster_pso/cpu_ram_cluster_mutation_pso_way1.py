...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  2 12:26:28 2018

@author: thieunv
- PSO update gbest sau moi~ lan 1 con chim di chuyen
...
#        sum = reduce(add, individual)  # sum = reduce( (lambda tong, so: tong + so), individual )
    ...
    def build_model_and_train(self):
        ...
        for i in range(self.move_count + 1):
            ...
            w = (self.move_count - i) / self.move_count * (w_max - w_min) + w_min
            for j in range(self.pop_size):
                r1 = np.random.random_sample()
                r2 = np.random.random_sample()
                vi_sau = w * pop[j][2] + c1*r1*( pop[j][1] - pop[j][0] ) + c2*r2*( gbest[0] - pop[j][0] )
...
w_max = 0.9
c1, c2 = 2, 2       # [0-2]   Muc do anh huong cua local va global 
# r1, r2 : random theo tung vong lap
# delta(t) = 1 (do do: x(sau) = x(truoc) + van_toc
pop_size = 100      # Kich thuoc quan the
move_count = 800     # So lan di chuyen` 
value_min = -1      # value min of weight 
value_max = +1
pso_param = [pop_size, move_count, value_min, value_max, w_min, w_max, c1, c2]
