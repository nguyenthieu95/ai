...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  6 21:39:16 2018

@author: thieunv

- PSO update gbest sau khi di chuyen toan bo ca dan chim
...
class Model(object):
    def __init__(self, dataset_original, list_idx, pso_param, sliding, positive_number, sti_level, dis_level = 0.25, method_statistic = 0):
        ...
        self.filenamesave = "point-slid_{0}-method_{1}-pos_{2}-sti_{3}-dis_{4}-pop_size_{5}-move_count_{6}-c1_{7}-c2_{8}".format(sliding, method_statistic, positive_number, sti_level, dis_level, pso_param[0], pso_param[1], pso_param[6], pso_param[7])
    ...
    def get_average_mae(self, weightHO, X_data, y_data):
        ...
        for i in range(0, len(X_data)):
            ...
            mae_list.append( abs( y_out - y_data[i] ) )
...
stimulation_levels = [0.25]  #[0.10, 0.2, 0.25, 0.50, 1.0, 1.5, 2.0]  # [0.20]    
...
w_max = 0.9
c_couple = [(2, 2)]         #c1, c2 = 2, 2       # [0-2]   Muc do anh huong cua local va global 
                    ...
                    for pop_size in pop_sizes:
                        for move_count in move_counts:
                            for c_id in c_couple:
                                ...
                                febpnn = Model(dataset_original, list_idx, pso_param, sliding, positive_number, sti_level, dis_level)
                                febpnn.fit()
