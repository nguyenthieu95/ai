...
- Version ko
    ...
    def get_mae(self, bee=None):
        ...
        for i in range(0, len(self.X_data)):
            output = np.add(np.matmul(self.X_data[i], w2), b2)
            y_pred = np.apply_along_axis(self.activation, 1, output)
            mae_list.append(np.average(np.power((self.y_data[i].flatten() - y_pred), 2)))
        temp = reduce(add, mae_list, 0)
        return temp / (len(self.X_data) * 1.0)
