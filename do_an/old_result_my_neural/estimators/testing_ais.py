...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 10:53:53 2018
...
#        print("done")
    ...
    def get_prj(self, repe_max, current_repe):
        ...
        for i in range(len(repe_max)):
            ...
            t2 = 1.0 /exp(t1) if t1 < 50 else 0.0
            list_p.append(t2)
    ...
    def receptor_editing(self, reperoite):
        temp = copy.deepcopy(reperoite)
        temp = temp + np.reshape(pow( (np.random.random_sample()), 3.0 ) * np.random.standard_cauchy(len(temp)), (-1, 1))
        return temp 
        
    
    def introduce_diverse_abs(self, list_promoted_ab):
        t = (self.rs - len(list_promoted_ab)) / 2
        temp = copy.deepcopy(list_promoted_ab)
        repertoires_new = []
        i = 0
        while(i < t):
            male = randint(0, len(list_promoted_ab) - 1)   # father
            female = randint(0, len(list_promoted_ab) - 1) # mother
            if male != female:
                father = copy.deepcopy(temp[male])
                mother = copy.deepcopy(temp[female])
                ...
                repertoires_new.append(copy.deepcopy(father))
                repertoires_new.append(copy.deepcopy(mother))
                i += 1

        return repertoires_new + list_promoted_ab
...
#        return sorted_repers[(len(new_repers)-self.rs):]
        new_repers = []
        for i in range(0, self.rs):
            x0 = self.get_affinity(repertoires[i])
            x1 = self.get_affinity(repertoires_new[i])
            temp = repertoires[i] if x0 > x1 else repertoires_new[i]
            new_repers.append( copy.deepcopy(temp) )
        repertoires = copy.deepcopy(new_repers)
...
#                    self.suppress()
            ...
            for pro_ab in list_promoted_ab:
                ...
                if np.random.random_sample() <= 0.5:
                    list_maturation.append( self.somatic_hypermutation(pro_ab, g) )
                else:
                    list_maturation.append( self.receptor_editing(pro_ab) )
            
            ### Step 5:
            repertoires_new = self.introduce_diverse_abs(list_maturation)     # Chon tren tap trung gian
...
p_rts = [0.92 ]        # [0.9, 0.95, 0.99]        # Xac suat nhan biet
reper_sizes = [20]               # [2, 4, 6]                  # Repertoire size (population size)
g_maxs = [50]      
